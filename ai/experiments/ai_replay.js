...
function makeReplay(scenarioName, options = {}) {
  ...
  function notifyStateChange(event = {}) {
    ...
    try {
      ...
      if (typeof options.onStateChangeError === "function") {
        options.onStateChangeError(error, { ...event });
      }
    }
  }
  ...
  const movementPhaseRecords = new Map();

  function movementPhaseRecord(side = state.active_side) {
    const kind = phaseKind();
    if (!["initial_movement", "mechanized_movement", "supply_movement"].includes(kind)) return null;
    const key = aiPhaseKey();
    if (!movementPhaseRecords.has(key)) {
      const eligible = unitsArray()
        ...
        .filter((unit) => Rules.canMoveInCurrentPhase(ctx(), unit))
        .map((unit) => unit.id);
      movementPhaseRecords.set(key, { key, eligible, acted: new Set() });
    }
    return movementPhaseRecords.get(key);
  }

  function movementPhaseSummary(side, override = null) {
    const record = movementPhaseRecord(side);
    if (!record) return null;
    const actedUnits = record.acted.size;
    const eligibleSet = new Set(record.eligible);
    const remaining = unitsArray().filter((unit) => eligibleSet.has(unit.id)
      && !record.acted.has(unit.id)
      && unit.state === "fresh"
      && Rules.canMoveInCurrentPhase(ctx(), unit)).length;
    return {
      movement_phase_policy: "rule_complete",
      eligible_units_at_phase_start: record.eligible.length,
      acted_units: actedUnits,
      held_units: Math.max(0, record.eligible.length - actedUnits),
      remaining_eligible_units: remaining,
      ...(override || {})
    };
  }
  ...
  function heuristicAiScore(action) {
    if (!action || action.type === "pass") return -Infinity;
    if (action.type === "combat") {
      const details = action.verdict?.details || {};
      const attack = Number(details.attack || 0);
      const defense = Math.max(1, Number(details.defense || 0));
      const odds = attack / defense;
      const crt = String(details.crt_column || "");
      const favorable = /AE|DE|EX/.test(crt) ? 20 : /DR|EX/.test(crt) ? 5 : 0;
      return odds * 10 + favorable - (odds < 2 ? 8 : 0);
    }
    if (action.type === "exit_west") return 100;
    if (action.type !== "move") return -100;
    const unit = state.units[action.unit];
    if (!unit?.hex || !action.path?.length) return -100;
    const destination = action.path.at(-1);
    const target = rulesAiMoveTarget(unit);
    const progress = distance(unit.hex, target) - distance(destination, target);
    const supply = aiScoreSupplyState(action.unit);
    const zoc = enemyZocSources(unit.side, destination).size;
    const mines = enemyMinesAt(unit.side, destination).length;
    return progress * 8
      + (supply === "unsupplied" ? -3 : 0)
      + (supply === "isolated" ? -8 : 0)
      - zoc * 2
      - mines * 6
      + (action.mode === "road" ? 0.5 : 0);
  }
  ...
  function suggestHeuristicAction() {
    ...
    try {
      const candidates = enumerateLegalAiActions(Math.max(80, Number(options.maxActions || 80)))
        .map((action) => ({ score: heuristicAiScore(action), ...action }))
        .sort((a, b) => b.score - a.score);
      const best = candidates.find((action) => action.type !== "pass");
      return {
        type: "heuristic",
        policy: "局部启发式：优先向当前阵营目标推进，避免雷区和敌方 ZOC，并选择赔率较高的相邻攻击。",
        target: fixedAiTarget(state.active_side),
        action: best || { type: "pass", reason: "heuristic ai pass" },
        candidates: candidates.slice(0, 10).map((action) => ({
          score: Number.isFinite(action.score) ? Number(action.score.toFixed(2)) : action.score,
  ...
  function autoPlay(maxSteps = 1000) {
    ...
    for (let step = 0; step < maxSteps && state.ai_autoplay; step += 1) {
      ...
      movementPhaseRecord(side);
      notifyStateChange({ stage: "before_decision", step: step + 1 });
      ...
      if (action.type === "pass") {
        const from = state.phase;
        const movement = movementPhaseSummary(side);
        advancePhase();
        log.push({
          step: step + 1,
          side,
          ...
          advance_reason: action.reason || "rules_ai_pass",
          movement_phase: movement ? { ...movement, advance_reason: action.reason || "rules_ai_pass" } : null
        });
        notifyStateChange({ stage: "phase_advance", step: step + 1 });
        continue;
      }
      ...
      if (result.legal) {
        ...
        if (["move", "exit_west"].includes(action.type)) movementPhaseRecord(side)?.acted.add(action.unit);
      }
  ...
  async function autoPlayWithProvider(providerOptions = {}) {
    ...
    const controllers = resolveControllers({
      controllers: providerOptions.controllers,
      axisController: providerOptions.axisController,
      alliesController: providerOptions.alliesController,
      externalSide: providerOptions.externalSide
    });
    state.player_control = { axis: controllers.axis, allies: controllers.allies };
    const externalAction = providerOptions.externalAction;
    const externalActions = providerOptions.externalActions || {};
    const hasExternalProvider = ["axis", "allies"].some((side) => controllers[side] === "external_ai");
    if (hasExternalProvider && typeof externalAction !== "function"
      && !Object.values(externalActions).some((value) => typeof value === "function")) {
      throw new Error("externalAction or externalActions provider is required for external_ai controller");
    }

    async function notifyExternalActionApplied(event = {}) {
      const callbackProvider = externalActions[event.side] || externalAction;
      if (typeof callbackProvider?.onActionApplied !== "function") return null;
      try {
        return await callbackProvider.onActionApplied({ ...event, state: clone(state) });
      }
      catch (error) {
        if (typeof options.onStateChangeError === "function") {
          options.onStateChangeError(error, { stage: "action_applied", step: event.step });
        }
    ...
    for (let step = 0; step < maxSteps && state.ai_autoplay; step += 1) {
      ...
      const source = controller === "external_ai" ? "external_model" : controller;
      if (controller === "human") {
        log.push({ step: step + 1, side, controller, source: "human", phase: state.phase, turn: state.turn, action: "waiting_for_human" });
      ...
      if (action.type === "pass") {
        ...
        await notifyExternalActionApplied({
          ...
          action: clone(action),
          result: { legal: true, phase_advanced: true }
        });
        notifyStateChange({ stage: "phase_advance", step: step + 1 });
        continue;
      }
      ...
      notifyStateChange({ stage: "after_action", step: step + 1 });
      if (!result.legal) {
        state.ai_autoplay = false;
        return { status: "illegal_action", log, steps: step + 1, result };
      }
  ...
  return {
    state,
    debugPhaseActionLimit(side = state.active_side) {
      return aiPhaseActionLimit(side);
    },
...
if (require.main === module) {
  ...
  fs.writeFileSync(prepareOutputFile(OUT), JSON.stringify(payload, null, 2));
