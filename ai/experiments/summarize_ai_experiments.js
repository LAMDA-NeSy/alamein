...
function metrics(transcript) {
  ...
  return {
    ...
    eligible_for_tactical_comparison: !infrastructureAffected
  };
}
...
function aggregate(items, key) {
  ...
  return [...groups.entries()].map(([value, rows]) => {
    ...
    const tacticalRows = rows.filter((row) => row.eligible_for_tactical_comparison);
    return ({
    [key]: value,
    runs: rows.length,
    ...
    raw_avg_victory_points: average(rows.map((row) => row.victory_points).filter(Number.isFinite)),
    tactical_comparison_runs: tacticalRows.length,
    infrastructure_affected_runs: rows.length - tacticalRows.length,
...
function summarizeTranscripts(transcripts) {
  ...
  return {
    ...
    attribution_rule: "Compare harnesses only when comparison_contract_hash, context_profile, and harness_prompt_hash match. The v6 contract also fixes rule-complete movement, request timeout, retry, circuit-breaker, and fallback-reserve policy. Runs with unresolved transport failures, circuit events, partial output, or harness errors retain raw VP but are excluded from tactical VP ranking.",
    runs,
