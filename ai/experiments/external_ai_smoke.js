...
const OUT = defaultLogFile("last_external_ai_smoke_report.json");
const CONTEXT_OUT = defaultLogFile("last_external_ai_context.json");
const AUDIT_OUT = defaultLogFile("last_external_ai_context_audit.json");
const MOCK_TRANSCRIPT_OUT = defaultLogFile("last_external_ai_mock_transcript.json");
const MOCK_EVAL_OUT = defaultLogFile("last_external_ai_mock_eval.json");
const QUALITY_OUT = defaultLogFile("last_external_ai_quality_report.json");
const ROLLOUT_OUT = defaultLogFile("last_external_ai_rollout_report.json");
const REQUIRE_API_EVAL_OUT = defaultLogFile("last_external_ai_require_api_eval.json");
...
function main() {
  ...
  run("syntax external_ai_transcript", ["-c", script("external_ai_transcript.js")]);
  run("syntax external_ai_eval", ["-c", script("external_ai_eval.js")]);
  run("syntax external_ai_context_audit", ["-c", script("external_ai_context_audit.js")]);
  run("syntax external_ai_quality", ["-c", script("external_ai_quality.js")]);
  run("syntax external_ai_rollout", ["-c", script("external_ai_rollout.js")]);
  run("syntax external_ai_real_check", ["-c", script("external_ai_real_check.js")]);
  run("syntax external_ai_probe", ["-c", script("external_ai_probe.js")]);
  run("context only", [script("external_ai_transcript.js"), "--context-only"]);
  run("fast context audit", [script("external_ai_context_audit.js")]);
  steps.push(moveIntentProbe());
  run("fast quality gate", [script("external_ai_quality.js")]);
  run("rollout gate", [script("external_ai_rollout.js")], { timeoutMs: 120000 });
  run("mock review transcript", [script("external_ai_transcript.js"), "--mock-review"]);
  run("mock eval", [
    script("external_ai_eval.js"),
    "--transcript", MOCK_TRANSCRIPT_OUT,
    "--out", MOCK_EVAL_OUT
  ]);
  run("require api rejects mock", [
    script("external_ai_eval.js"),
    "--transcript", MOCK_TRANSCRIPT_OUT,
    "--out", REQUIRE_API_EVAL_OUT,
    "--require-api"
  ], { expectFailure: true });
  run("rule engine tests", [path.join(ROOT, "rule_engine.test.js")]);

  const artifacts = {};
  for (const [key, file] of Object.entries({
    context: CONTEXT_OUT,
    audit: AUDIT_OUT,
    mock_transcript: MOCK_TRANSCRIPT_OUT,
    mock_eval: MOCK_EVAL_OUT,
    quality: QUALITY_OUT,
    rollout: ROLLOUT_OUT,
    require_api_eval: REQUIRE_API_EVAL_OUT
  })) {
  ...
  fs.writeFileSync(prepareOutputFile(OUT), JSON.stringify(report, null, 2));
