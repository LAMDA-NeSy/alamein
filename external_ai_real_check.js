...
const TRANSCRIPT_OUT = path.join(ROOT, "last_external_ai_transcript.json");
const EVAL_OUT = path.join(ROOT, "last_external_ai_eval.json");
const QUALITY_OUT = path.join(ROOT, "last_external_ai_real_quality.json");
const REPORT_OUT = path.join(ROOT, "last_external_ai_real_check.json");
...
function main() {
  ...
  const transcriptOut = path.resolve(argValue("--transcript-out", TRANSCRIPT_OUT));
  const evalOut = path.resolve(argValue("--eval-out", EVAL_OUT));
  const qualityOut = path.resolve(argValue("--quality-out", QUALITY_OUT));
  const reportOut = path.resolve(argValue("--out", REPORT_OUT));
  const scenario = argValue("--scenario", "");
  const phase = argValue("--phase", "");
  const activeSide = argValue("--active-side", "");
  const turn = argValue("--turn", "");
  const transcriptArgs = ["external_ai_transcript.js", "--out", transcriptOut];
  ...
  const qualityArgs = ["external_ai_quality.js", "--out", qualityOut];
